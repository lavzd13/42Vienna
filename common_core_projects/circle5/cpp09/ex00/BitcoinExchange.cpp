...
int	BitcointExchange::parse_date(std::string &date) {
	...
	for (size_t i = 0; i < date.size(); i++) {
		if (!std::isdigit(date[i])) {
			if (date[i] == ' ' && spaceCount == 1)
				spaceCount--;
			else if (date[i] == '-' && minusCount > 0)
				minusCount--;
			else
				return 1;
		}
...
void	BitcointExchange::parse_input_file(std::ifstream &input_file) {
	...
	while (std::getline(input_file, line)) {
		...
		int	dateCheck = parse_date(date);
		int	valueCheck = parse_value(value);
		if (dateCheck == 1 || valueCheck == 1 || valueCheck == 2) {
			if (valueCheck == 2 || dateCheck == 1)
				std::cout << "Error: bad input => " + line << std::endl;
		}
		else
